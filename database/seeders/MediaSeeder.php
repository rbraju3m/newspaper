...
class MediaSeeder extends Seeder
{
    ...
    private function library(ImageService $service, ?int $userId): array
    {
        ...
        foreach ($sections as $section) {
            for ($variant = 1; $variant <= self::VARIANTS; $variant++) {
                ...
                $library[$section->slug][] = Media::query()->where('filename', $filename)->first()
                    ?? $this->store(
                        $service,
    ...
    private function assignToAds(ImageService $service, ?int $userId): void
    {
        ...
        $ads = Ad::query()->where('type', 'image')->whereNull('asset')->orderBy('id')->get();
        ...
        foreach ($ads as $i => $ad) {
            ...
            $media = Media::query()->where('filename', $filename)->first()
                ?? $this->store(
                    $service,
        ...
        if ($ads->isNotEmpty()) {
            $this->command->info("Filled {$ads->count()} ad slots.");
        }
    }
