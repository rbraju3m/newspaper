...
<div x-data x-cloak x-show="$store.push.supported" {{ $attributes }}>
    <template x-if="$store.push.blocked">
        <p @class([
            ...
            <span>{{ __('নোটিফিকেশন ব্লক করা আছে। ব্রাউজারের সাইট সেটিংস থেকে অনুমতি দিন।') }}</span>
        </p>
    </template>
    ...
    <template x-if="! $store.push.blocked">
        <button type="button"
            ...
            <span x-text="$store.push.subscribed
                ? '{{ __('ব্রেকিং অ্যালার্ট চালু আছে') }}'
                : '{{ __('ব্রেকিং নিউজ অ্যালার্ট পান') }}'">{{ __('ব্রেকিং নিউজ অ্যালার্ট পান') }}</span>
        </button>
    </template>
</div>
