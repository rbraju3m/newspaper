    ...
    <div class="border-b border-line bg-surface-2">
        <div class="mx-auto flex max-w-site flex-col items-center gap-4 px-4 py-8 md:flex-row md:justify-between">
            <div class="text-center md:text-left">
                <h2 class="font-headline text-xl font-bold text-ink">{{ __('দিনের সেরা খবর ইনবক্সে') }}</h2>
            ...
            <form action="{{ route('newsletter.subscribe') }}" method="POST"
                ...
                <input id="nl-email" type="email" name="email" required placeholder="{{ __('আপনার ইমেইল ঠিকানা') }}"
    ...
    <div class="mx-auto max-w-site px-4 py-10">
        <div class="grid grid-cols-2 gap-8 md:grid-cols-4 lg:grid-cols-5">
            <div class="col-span-2 lg:col-span-1">
                ...
                <p class="mt-3 text-sm leading-relaxed text-muted">{{ \App\Support\Locale::siteDescription() }}</p>
            ...
            @foreach ($allCategories->take(24)->chunk(8) as $chunk)
                <nav aria-label="{{ __('বিভাগ') }}">
                    <h3 class="font-headline text-sm font-bold uppercase tracking-wide text-ink">{{ __('বিভাগ') }}</h3>
            ...
            <nav aria-label="{{ __('প্রতিষ্ঠান') }}">
                <h3 class="font-headline text-sm font-bold uppercase tracking-wide text-ink">{{ __('প্রতিষ্ঠান') }}</h3>
                <ul class="mt-3 space-y-2">
                    @foreach ([
                        'about' => __('আমাদের সম্পর্কে'),
                        'contact' => __('যোগাযোগ'),
                        'advertise' => __('বিজ্ঞাপন'),
                        'privacy' => __('গোপনীয়তা নীতি'),
                        'terms' => __('ব্যবহারের শর্তাবলি'),
                        'comment-policy' => __('মন্তব্য নীতি'),
                    ] as $slug => $label)
                        <li>
        ...
        <div class="mt-8 border-t border-line pt-6 text-sm text-muted">
            @if (config('site.editor'))
                <p><span class="font-semibold text-body">{{ __('সম্পাদক:') }}</span> {{ config('site.editor') }}</p>
            @endif
            @if (config('site.address'))
                <p class="mt-1">{{ config('site.address') }}</p>
            @endif
            <p class="mt-1">
                @if (config('site.phone')) {{ __('ফোন:') }} @bn(config('site.phone')) @endif
                @if (config('site.email')) · {{ __('ইমেইল:') }} {{ config('site.email') }} @endif
            </p>
        </div>

        <div class="mt-6 flex flex-col items-center justify-between gap-2 border-t border-line pt-5
                    text-xs text-muted sm:flex-row">
            <p>© @bn(date('Y')) {{ \App\Support\Locale::siteName() }} — {{ __('সর্বস্বত্ব সংরক্ষিত।') }}</p>
            <p>{{ __('ডেভেলপ করেছে') }} <span class="font-medium text-body">{{ config('site.name_en') }}</span></p>
        </div>
    </div>
</footer>
