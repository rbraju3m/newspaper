...
| A photo gallery is one row shown in two editions; a video is not | Two neighbouring surfaces follow opposite rules | A video is an article — words, and words belong to an edition. A gallery is photographs, which are the same from either side, so it has one slug and two frames like an e-paper issue. The question for any new surface is what the reader is actually consuming |
| `VideoController::show()` gained a canonical redirect it never had | One more 301 on a route that used to just render | The id lookup is locale-blind, so `/en/video/{bangla id}` resolved and rendered a Bangla video inside an English frame at a URL claiming to be English. `ArticleController` has had this guard all along; the video route is the same shape and needed the same thing |
| A subscriber's edition is a column, captured at sign-up | One more thing that can be wrong, and a reader cannot change it without unsubscribing | The digest is sent by cron hours later, from a process standing on no URL. The only moment anybody knows which language this reader wanted is the moment they typed their address into a box that was already in it |
| `newsletter:send` switches locale for a whole subscriber's turn | Global state mutated inside a loop, which is the shape that usually goes wrong | Article selection, subject line and mail rendering are one decision and all three read the ambient locale. Doing it three times separately is three chances to disagree. The `finally` is what makes it safe — a throw would otherwise leave every remaining reader in the failed one's language |
| The account preferences screen does not write `locale` | An English subscriber who edits their frequency in the Bangla account area keeps a column that screen cannot show them | It does not ask about language, so it must not answer the question. Writing the request's edition there would silently move them back to Bangla on every save, and the failure is invisible — they simply stop getting the mail they chose |
