...
| The fix went to the digest and not to the site's four other colour labels | The same defect is still on the reader-facing pages | Those render on a dark theme as well as a light one, and a colour darkened for white is the wrong move on `.dark`. `readable()` takes the background it is asked about, so the mechanism is there; the decision about what dark should show is not, and guessing it inside an email fix is how a scoped change becomes a redesign |
